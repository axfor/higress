...
- the automatic conversion of Claude protocol input (`/v1/messages` sent to an OpenAI-compatible provider without native Anthropic support): two transformers in series inside the engine, Claude → OpenAI first (each message held whole, bounded, and converted by the buffered rules; a `messages` array ahead of `system` is held, bounded), then the provider's own transform -- the provider's chat plan with the conversion stage in front (zhipuai, openrouter, azure, minimax, gemini, vertex and the rest alike; with `mergeConsecutiveMessages` on, the merge stage sits between them). zhipuai and openrouter read the thinking type and budget the buffered path keeps from the Claude body; the conversion stage hands them to the variant for its tail;
...
- embeddings for `gemini` (the `input` array streams element by element into `requests`; each element names the model, so an `input` ahead of `model` is held, bounded), `vertex` and native `qwen` (a non-string element is rejected as on the buffered path); `gemini` `/v1/images/generations`; `vertex` `/v1/images/generations` / `edits` / `variations` (JSON bodies; an image input's data URL streams out as `inlineData` while it arrives, an http(s) URL must fit in 8KB, the prompt is written last; with both `images` and `image` given and `image` first the parts keep arrival order, and an input naming both a non-empty `url` and `image_url` is rejected rather than resolved to the latter; multipart bodies keep the buffered path);
...
- bedrock Converse chat with apiTokens (the full OpenAI → Converse mapping: system blocks, toolUse / toolResult blocks and their merging, data-URL images, inferenceConfig / thinking / toolConfig; falls back when prompt caching is configured for a supporting model, since cache points go on the last user message);
...
- `customSettings`: the first stage of the pipeline, ahead of the provider conversion (the buffered path rewrites the body before the handler too); an overwriting setting is written as soon as its container starts (the request's own value is dropped), an add-if-absent one waits for the container to close, and missing intermediate objects are created as sjson creates them. Only dotted plain keys are taken (array indexes, escapes, wildcards and other gjson syntax fall back), and an array on the way to a setting falls back too (sjson fails there).
- `responseJsonSchema` (openai / longcat): the configured schema replaces the request's `response_format`; the buffered path also re-serialises the whole request through its struct on the way (fields the struct lacks and zero-valued optional fields are dropped, `stream_options.include_usage: false` turns into true, numbers are rendered as `json.Marshal` renders them), which streaming reproduces with a struct round-trip stage driven by the type tree derived from the request struct: keys without a field dropped at every level, the zero values of omitempty fields left out, the zero values of the other fields written when absent, numbers re-rendered; a subtree the struct takes as an interface only has its numbers re-rendered.
- `context`: once the file content is cached (the first request takes the buffered path, which fetches it), chat requests stream with the insertion: an OpenAI-shaped body (the OpenAI-compatible family, azure, qwen compatible, zhipuai, openrouter, minimax v2, vertex's OpenAI-compatible endpoint, and the converted Claude-protocol input) gets the system message before the first non-system message, for which each message's keys ahead of its role are held (64KB); claude gets the content in front of system (`content\nsystem`). The insertion is followed by the same struct round trip (the buffered inserter re-serialises through `chatCompletionRequest`). Deviations: when every message is system the buffered path prepends the message and streaming, having released them, appends it; when there is no system message the buffered claude insertion dereferences a nil pointer, streaming makes the content the system prompt. Chat on any other body shape (gemini, vertex's Gemini shape, bedrock, ...) keeps the buffered path.
- `mergeConsecutiveMessages` (and `hiclawMode`): a pipeline stage ahead of the provider conversion and behind the Claude-protocol input conversion, where the buffered path runs it; each message is held whole (1MB) until the next one's role says whether they merge, by the buffered rules (including the behaviour that a merge onto already merged parts keeps only the newer parts); a message never merged passes through untouched; the buffered path's re-serialisation of the whole request after a merge is not reproduced. A missing `image_url.url` and the other fields the buffered path asserts without checking fall back (the buffered path panics).
- native Qwen's `qwenFileIds` (when the mapped model is qwen-long) and `context`: the file list / file content goes in as a system message before the first non-system message, after a "You are a helpful assistant." message when that is the first one; `qwenFileIds` folds the leading system messages into one (texts joined by newlines). Each message's keys ahead of its role are held (64KB); `qwenFileIds` needs `model` before `messages` (whether to insert depends on the model). Deviation: when every message is system the buffered path puts the two added messages first, streaming appends them (in the folding mode the folded system messages are written back after them).
- multipart bodies (`/v1/images/edits`, `/v1/images/variations` on the providers of the default conversion): the buffered path only maps the `model` field -- the body untouched when the mapping changes nothing, re-encoded whole with Go's multipart writer when it does; streaming keeps both renderings side by side until the `model` field is read (within the commit window), releases the one the buffered path would have produced, and streams the image parts after it. `model` must be within the window (SDKs put the text fields before the files); a second `model` field, a header block over 8KB, a `model` value over 4KB and a bare-LF body fall back, as does a multipart request under `customSettings` (the buffered path's sjson on a form is not reproduced). Vertex and Gemini parse the form into their own JSON request and keep the buffered path.

Other providers (Bedrock in AK/SK mode, Hunyuan) and requests configured with `contextCleanupCommands` / `retryOnFailure` keep using the buffered path unchanged. `firstByteTimeout` works on chat as before (`stream` must appear within the window, otherwise the request falls back); `providerBasePath` works for every provider.
