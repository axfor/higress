...
- embeddings for `gemini` (the `input` array streams element by element into `requests`; each element names the model, so an `input` ahead of `model` is held, bounded), `vertex` and native `qwen` (a non-string element is rejected as on the buffered path); `gemini` `/v1/images/generations`;
