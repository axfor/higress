...
Other providers (Bedrock in AK/SK mode, Hunyuan) and requests configured with `contextCleanupCommands` / `retryOnFailure` / `qwenFileIds` (native qwen) keep using the buffered path unchanged. `firstByteTimeout` works on chat as before (`stream` must appear within the window, otherwise the request falls back); `providerBasePath` works for every provider.
